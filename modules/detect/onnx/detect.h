...
#ifndef YOLOV8_DEPLOY_MODULES_DETECT_ONNX_DETECT_H_
#define YOLOV8_DEPLOY_MODULES_DETECT_ONNX_DETECT_H_
...
namespace detect::onnx {

class Detect {
 public:
  Detect() : ortMemoryInfo_(Ort::MemoryInfo::CreateCpu(OrtAllocatorType::OrtDeviceAllocator,
  ...
  bool LoadModel(const std::string &modelPath,
    ...
    try {
      ...
      if (isCuda && warmUp) {
        ...
        size_t input_tensor_length = segment::VectorProduct(inputTensorShape_);
  ...
  bool BatchDetect(std::vector<cv::Mat> &srcImages,
                   std::vector<InferenceResults> &outputs,
    ...
    int64_t input_tensor_length = segment::VectorProduct(inputTensorShape_);
    std::vector<Ort::Value> input_tensors;
    std::vector<Ort::Value> output_tensors;
    ...
    int net_width = outputTensorShape_[1];
    int socre_array_length = net_width - 4;
    int64_t one_output_length = segment::VectorProduct(outputTensorShape_) / outputTensorShape_[0];
    for (int img_index = 0; img_index < srcImages.size(); ++img_index) {
      ...
      auto *detections = (float *) output0.data;
      int rows = output0.rows;
      std::vector<int> class_ids;
      std::vector<float> confidences;
      std::vector<cv::Rect> boxes;
      for (int r = 0; r < rows; ++r) {    //stride
        cv::Mat scores(1, socre_array_length, CV_32F, detections + 4);
        ...
        if (score >= classThreshold_) {
          ...
          //rect [x,y,w,h]
          float x = (detections[0] - params[img_index][2]) / params[img_index][0];  //x
          float y = (detections[1] - params[img_index][3]) / params[img_index][1];  //y
          float w = detections[2] / params[img_index][0];  //w
          float h = detections[3] / params[img_index][1];  //h
 ...
 private:
  //ONNXRUNTIME
  Ort::Env ortEnv_ = Ort::Env(OrtLoggingLevel::ORT_LOGGING_LEVEL_ERROR, "Yolov8-Detect");
...
#endif //YOLOV8_DEPLOY_MODULES_DETECT_ONNX_DETECT_H_
