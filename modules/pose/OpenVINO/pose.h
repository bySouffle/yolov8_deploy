...
#ifndef YOLOV8_DEPLOY_MODULES_POSE_OPENVINO_POSE_H_
#define YOLOV8_DEPLOY_MODULES_POSE_OPENVINO_POSE_H_
 ...
 public:
  ...
  bool BatchDetect(std::vector<cv::Mat> &srcImages, std::vector<InferenceResults> &outputs, bool countTime = false) {
    ...
    for (int i = 0; i < input_images.size(); ++i) {
      ...
      int key_point_length = output_shape[1] - 5;
      int key_point_num = 17;
      ...
      for (int j = 0; j < det_output.cols; ++j) {
        float score = det_output.at<float>(4, j);
        if (score > classThreshold_) {
          ...
          for (int k = 0; k < key_point_num; ++k) {
            ...
            temp_kp.x = (det_output.at<float>(k * 3 + 5, j) - params[i][2]) / params[i][0];
            temp_kp.y = (det_output.at<float>(k * 3 + 6, j) - params[i][3]) / params[i][1];
            temp_kp.confidence = det_output.at<float>(k * 3 + 7, j);
...
#endif //YOLOV8_DEPLOY_MODULES_POSE_OPENVINO_POSE_H_
